Here is the rewritten script using `Pathlib` and `f-strings`, along with other improvements:
...
### Improvements Made:
1. **Pathlib**: Used `Pathlib` to handle file and directory paths.
2. **f-strings**: Replaced old-style string formatting with f-strings.
3. **Code Clean-up**: Removed commented-out lines and improved the code readability.
4. **Default Argument Handling**: Changed the default `search` argument to a list to properly handle nargs in argparse.
